...
class MainMenuWindow(ttk.Frame, ThemeEngine):
    ...
    def __init__(self, master):
        ...
        # Settings Button
        self.Settings_btn_img = tk.PhotoImage(file = r"images/Settings_button.png")
        Settings_button = tk.Button(body_frame , text="Settings", image=self.Settings_btn_img,
                                 bd=0, bg=self.button_bg, activebackground=self.button_bg,
                        command=lambda: master.switch_frame(SettingsWindow, bg_frame))
        Settings_button.grid(row=0, column=2, padx=30)
